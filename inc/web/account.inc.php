...
if($_W['role']=='operator'){
    //查找商家ID;
    $account=pdo_get('zhtc_account',array('weid'=>$_W['uniacid'],'uid'=>$_W['user']['uid']));
...
" . ($pindex - 1) * $psize . ',' . $psize, $data);
    ...
    if (!empty($list)) {
        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('zhtc_account') . " WHERE weid = :weid $strwhere", array(':weid' =>$_W['uniacid']));
        $pager = pagination($total, $pindex, $psize);
    }
} else if ($operation == 'delete') {
    $id = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id FROM " . tablename('zhtc_account') . " WHERE id = '$id'");
    if (empty($item)) {
