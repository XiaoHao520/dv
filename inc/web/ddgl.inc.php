...
if($operation=='refund'){
   ...
   // var_dump($result);die;
    ...
    //更改订单操作
        pdo_update('zhtc_order',array('status'=>6),array('id'=>$id));           
...
/*if(checksubmit('export_submit', true)) {
    ...
    $time="'%$time%'";
        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("zhtc_order")." WHERE time LIKE ".$time);
        ...
        for ($j = 1; $j <= $pagesize; $j++) {
            $sql = "select a.*,b.store_name as seller_name from " . tablename("zhtc_order")."  a"  . " inner join " . tablename("zhtc_store")." b on a.store_id=b.id  WHERE a.time LIKE ".$time."  limit " . ($j - 1) * 5000 . ",5000 ";
