...
$GLOBALS['frames'] = $this->getMainMenu();
	$info = pdo_get('zhtc_yellowtype',array('uniacid' => $_W['uniacid'],'id'=>$_GPC['id']));
		if(checksubmit('submit')){
			...
			if($_GPC['id']==''){				
				$res=pdo_insert('zhtc_yellowtype',$data);
				if($res){
				...
				$res = pdo_update('zhtc_yellowtype', $data, array('id' => $_GPC['id']));
				if($res){
